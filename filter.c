...
)
{
	...
	PMDL pMdl = NULL;
	PUCHAR pCurrentBuffer = NULL;
	ULONG BytesAvailable = 0;
	ULONG mdlOffset = 0;
	ULONG SrcTotalLenght = 0;
	ULONG bytesToCopy = 0;

	PUCHAR pCopyBuffer = NULL;
	UINT           BytesCopied = 0;

	UCHAR* pPackage = NULL;
	NDISPROT_ETH_HEADER* pEthHeader = NULL;
	IPV4_HDR* pIPHeader = NULL;
	...
	while (pNetBufList)
	{
		pNetBuf = NET_BUFFER_LIST_FIRST_NB(pNetBufList);
		while (pNetBuf)
		{
			pMdl = NET_BUFFER_CURRENT_MDL(pNetBuf);
			mdlOffset = NET_BUFFER_CURRENT_MDL_OFFSET(pNetBuf);
			SrcTotalLenght = NET_BUFFER_DATA_LENGTH(pNetBuf);

			pCopyBuffer = ExAllocatePoolWithTag(NonPagedPool, DestBufferSize, FILTER_TAG);
			if (NULL == pCopyBuffer)
			{
				DEBUGP(DL_ERROR, ("NdisAllocateMemoryWithTag Failed \n"));
				return;
			}

			NdisZeroMemory(pCopyBuffer, DestBufferSize);
			BytesCopied = 0;
			bytesToCopy = 0;

			while (pMdl)
			{
				NdisQueryMdl(pMdl, &pCurrentBuffer, &BytesAvailable, NormalPagePriority);
				if (NULL == pCurrentBuffer)
				{
					DEBUGP(DL_ERROR, ("pData is NULL! \n"));
					break;
				}

				pPackage = pCurrentBuffer + mdlOffset;

				bytesToCopy = BytesAvailable - mdlOffset;
				if (bytesToCopy > SrcTotalLenght)
					bytesToCopy = SrcTotalLenght;

				NdisMoveMemory(pCopyBuffer + BytesCopied, pPackage, bytesToCopy);
			...
			pEthHeader = (NDISPROT_ETH_HEADER*)(pCopyBuffer);
			...
			{
				//				DEBUGP(DL_ERROR, ("ETHERTYPE_ARP \n"));
			}
			else if (pEthHeader->EthType == ETHERTYPE_REVARP)
			{
				//				DEBUGP(DL_ERROR, ("ETHERTYPE_REVARP \n"));
			}

			if (NULL != pCopyBuffer)
			{
				ExFreePoolWithTag(pCopyBuffer, FILTER_TAG);
				pCopyBuffer = NULL;
			}
...
--*/
{
	...
	do
	{
		...
		FILTER_ACQUIRE_LOCK(&pFilter->Lock, DispatchLevel);
		OnPackageDispatch(NetBufferLists, TRUE);
		FILTER_RELEASE_LOCK(&pFilter->Lock, DispatchLevel);
...
--*/
{
	...
	do
	{
		...
		FILTER_ACQUIRE_LOCK(&pFilter->Lock, DispatchLevel);
		OnPackageDispatch(NetBufferLists, FALSE);
		FILTER_RELEASE_LOCK(&pFilter->Lock, DispatchLevel);
